...
SELECT E.storeno, E.ordno, E.prdno, E.grade, qtty, ROUND(IF(I.last_cost = 0, I.cm_varejo_otn, I.last_cost)) / 100 AS 
  cost, V.no AS vendno, C.no AS custno, E.empno 
FROM sqldados.eoprd AS E 
    INNER JOIN sqldados.eord AS O 
    ON O.ordno = E.ordno AND
    O.storeno = E.storeno 
    INNER JOIN sqldados.stk AS I 
    ON I.storeno = E.storeno AND
    I.prdno = E.prdno AND
    I.grade = E.grade 
...
SELECT @INVNO AS invno , vendno , ordno , 0 AS xfrno , CURRENT_DATE * 1 AS issue_date , CURRENT_DATE * 1 AS DATE , 
  CURRENT_DATE * 1 AS comp_date , 0 AS ipi , 0 AS icm , 0 AS freight , 0 AS netamt , SUM( qtty * cost / 1000 ) AS 
  grossamt , 0 AS subst_trib , 0 AS discount , SUM( qtty * cost / 1000 ) AS prdamt , 0 AS despesas , 0 AS base_ipi , 0 
  ...
  0 AS s1 , 0 AS s2 , 0 AS s3 , 0 AS s4 , 0 AS s5 , 0 AS s6 , 0 AS s7 , 0 AS s8 , @NFNO AS nfname , @SERIE AS invse , 2 
...
SELECT @INVNO AS invno , qtty , cost AS fob , cost , CURRENT_DATE * 1 AS DATE , 0 AS ipi , 0 AS auxLong1 , 0 AS auxLong2 
  , 0 AS frete , 0 AS seguro , 0 AS despesas , 0 AS freteIpi , 0 AS qttyRessar , 0 AS baseIcmsSubst , 0 AS icmsSubst , 0 
  AS icms , 0 AS discount , cost * 100 AS fob4 , cost * 100 AS cost4 , 0 AS icmsAliq , 1949 AS cfop , 0 AS auxLong3 , 0 
...
SELECT @XANO AS xano , @NFNO AS nfno , custno , CURRENT_DATE * 1 AS issuedate , CURRENT_DATE * 1 AS delivdate , 0 AS 
  sec_amt , 0 AS fre_amt , 0 AS netamt , SUM( qtty * cost / 1000 ) AS grossamt , 0 AS discount , 0 AS icms_amt , 0 AS 
  ...
  AS s5 , 0 AS s6 , 0 AS s7 , 0 AS s8 , @SERIE AS nfse , '' AS ship_by , '' AS vol_make , '' AS vol_kind , '66' AS 
  remarks , '' AS padbyte , '' AS print_remarks , '' AS remarksCancel , @DOC AS c1 , '' AS c2 , '' AS wshash 
FROM T 
WHERE @TIPO = 'S' 
GROUP BY storeno, ordno;
...
SELECT @XANO AS xano , @NFNO AS nfno , cost AS price , CURRENT_DATE * 1 AS DATE , qtty / 1000 AS qtty , storeno , 0 AS 
  pdvno , prdno , grade , @SERIE AS nfse , '' AS padbyte , @DOC AS wshash 
FROM T 
WHERE @TIPO = 'S';
...
INSERT 
INTO sqldados.lastno( NO, storeno, dupse, se, padbyte) 
VALUES
  (
    @NFNO , @LOJA , 0 , @SERIE , '' 
  )
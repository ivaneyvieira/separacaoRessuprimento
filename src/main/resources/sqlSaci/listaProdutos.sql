SELECT O.prdno,
       ...
       IFNULL(LS.localizacao, IF(LENGTH(O.auxStr) > 20, '', O.auxStr)) AS localizacaoSaci,
       P.typeno                                                        AS tipo,
...
FROM sqldados.oprd AS O
       ...
       LEFT JOIN sqldados.prdloc AS LS
                 ON (O.prdno = LS.prdno AND O.grade = LS.grade AND
                     LS.storeno = 4 AND (LENGTH(O.auxStr) > 20 OR O.auxStr = ''))

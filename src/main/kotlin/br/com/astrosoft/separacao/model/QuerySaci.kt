...
class QuerySaci : QueryDB(driver, url, username, password) {
  ...
  fun listaPedido(): List<Pedido> {
    ...
    val ret = query(sql) { q ->
      q.addParameter("storeno", storeno)
      q.addParameter("storenoAux", storenoAux)
  ...
  fun listaPedidoPendente(): List<Pedido> {
    ...
    val ret = query2(sql, Pedido::class) {
      addOptionalParameter("storeno", storeno)
      addOptionalParameter("storenoAux", storenoAux)
    }
    return ret
  }
  ...
  fun atualizarQuantidade(
    ordno: Int, ordnoNovo: Int, codigo: String, grade: String, localizacao: String, qtty: Int,
    tipo: ETipoOrigem
  ) {
    val storeno = 1
